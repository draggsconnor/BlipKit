...
The constants `BK_C_0` to `BK_C_8` are values between 0 and 96 and only used for readability. Of course, their equivalent integer values can be used as well.
...
Audio data is generated by requesting an arbitary number of *frames*. This is done by calling either [BKContextGenerate](../context-object/#bkcontextgenerate), which fills a provided buffer with the requested frames, or its variant [BKContextGenerateToTime](../context-object/#bkcontextgeneratetotime), which outputs the frames to a provided callback.
...
<p>A <a href="../waveforms/">custom waveform</a> can be set with the the pointer setter function:</p>
...
<p>Raises or lowers the track's pitch. The value is added to the <code>BK_NOTE</code> attribute. Default is 0.</p>
